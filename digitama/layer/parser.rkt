...
(define parse-layer-record : (-> Bytes Fixnum Positive-Byte Positive-Byte (values PSD-Layer-Record PSD-Layer-Infobase Fixnum))
  (lambda [layer-info idx ps-size channel-step]
        ...
        (cond [(fx= channel-No. channel-count) (values (reverse slennahc) channel-start)]
              [else (cons-info (fx+ channel-No. 1)
                               ...
                               (cons (cons (parse-int16 layer-info channel-start)
                                           (parse-size layer-info (fx+ channel-start 2) ps-size))
                                     slennahc))])))
    ...
    (values (PSD-Layer-Record x y width height channels blend opacity base-clipping? flags mask blending-ranges pascal-name)
            infobase end-idx #;(fx+ (fx+ 8BIM-idx 16) total:mask+|blending-ranges|+|pascal-name|+infobase))))
