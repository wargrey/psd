...
(module unsafe racket/base
  ...
  (define (planar-data->bitmap planar-data width height channels density)
    ...
    (if (bytes? planar-data)
        (case channels
          [(3) (fill-rgba-from-rgba! pixels planar-data width height #false)]
          [(4) (fill-rgba-from-rgba! pixels planar-data width height #true)])
        (case channels
          [(3) (fill-rgba-from-rgba*! pixels planar-data width height channels)]
          [(4) (fill-rgba-from-rgba*! pixels planar-data width height channels)]))
  ...
  (define (fill-rgba-from-rgba! pixels source width height alpha?)
    (define total (unsafe-fx* width height))
    (let fill! ([idx 0])
      (when (unsafe-fx< idx total)
        (define dest-idx (unsafe-fx* idx 4))
  ...
  (define (fill-rgba-from-rgba*! pixels sources width height channels)
    (let fill! ([rest sources] [row 0] [channel-idx 0])
      (when (unsafe-fx< channel-idx channels)
        ...
        (define offset (unsafe-fx* row width))
        (define src (unsafe-car rest))
        (let subfill! ([col 0])
          (when (unsafe-fx< col width)
            (define dest-idx (unsafe-fx* (unsafe-fx+ offset col) 4))
